...
if (!class_exists('WP_Sheet_Editor_Columns_Renaming')) {
	...
	class WP_Sheet_Editor_Columns_Renaming {
		...
		function add_renaming_options($sections) {
			...
			$pts = VGSE()->helpers->get_all_post_types_names();
			$labels = array();
			$labels[] = array(
				'id' => 'info_normal',
				'type' => 'info',
			...
			foreach ($pts as $post_type) {
				...
				$post_type_label = VGSE()->helpers->get_post_type_label($post_type);
				...
				foreach ($spreadsheet_columns as $key => $column) {
					...
					if ($column['allow_to_rename']) {
						...
						if (in_array($key, $taxonomies)) {
							$extra_desc = ( $post_type === 'product' && strpos($key, 'pa_') !== false ) ? ' (Product Attribute)' : '';
							...
							if ($post_type === 'product') {
								$field_args['required'] = array(array('be_post_types', '=', 'product'));
							}
							$labels[] = $field_args;
						} else {
							$labels[] = array(
